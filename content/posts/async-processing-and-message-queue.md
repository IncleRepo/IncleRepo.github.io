...
lastmod = 2026-08-10T16:35:55+09:00
draft = false
...
Annotation 하나를 붙였다고 Java 메서드 안에서 갑자기 Thread가 바뀌는 것은 아니다. 다른 Spring Bean이 이 메서드를 호출하면 Spring이 만든 Proxy가 실제 Bean보다 먼저 호출을 받는다. Proxy는 메서드를 직접 실행하지 않고 실행할 작업을 `TaskExecutor`에 제출한다.
