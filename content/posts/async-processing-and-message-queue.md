...
lastmod = 2026-08-10T16:43:23+09:00
draft = false
...
물론 모든 작업을 분리할 수 있는 것은 아니다. 결제 승인처럼 그 결과에 따라 주문 성공 여부가 달라진다면 HTTP 응답 전에 결과를 확인해야 한다. 반면 주문 완료 이메일처럼 실패하더라도 주문 저장 결과가 바뀌지 않는 부수 작업은 요청과 분리할 수 있다. 비동기 처리를 검토할 때는 먼저 **이 작업의 결과가 현재 응답에 꼭 필요한지**를 판단해야 한다.
...
- 작업이 끝날 때까지 호출자가 결과를 기다리는가? 이는 동기와 비동기를 나누는 기준이다.
...
Annotation 하나를 붙였다고 Java 메서드 안에서 갑자기 Thread가 바뀌는 것은 아니다. `@Async`는 Spring이 관리하는 Bean과 그 앞에 만들어 둔 Proxy를 통해 동작한다.
...
이 처리 순서를 기준으로 각 설정의 역할을 살펴보면 다음과 같다.
...
앞의 설정에서 이메일 작업 500개가 거의 동시에 들어오고, 처리 중인 작업이 아직 하나도 끝나지 않았다고 단순하게 가정해 보자.
...
이 상황에서는 사용자에게 이미 `200 OK`가 반환됐더라도 이메일이 발송되지 않을 수 있다. 재시작한 애플리케이션은 이전 프로세스의 메모리 Queue에 어떤 작업이 있었는지 알 수 없다. “응답을 먼저 보낸다”는 장점을 얻는 대신, 작업을 복구할 근거가 사라질 수 있는 것이다.
...
Message Queue
→ 작업이 Application 외부의 Broker에 기록됨
```
...
주문 서비스는 이메일 메서드를 직접 실행해 달라고 요청하지 않는다. 대신 “주문이 완료됐다”는 사실을 Event로 기록한다. 이메일 Consumer는 그 Event를 읽은 뒤 자신의 실행 환경에서 이메일을 보낸다. 실행할 Java 코드를 Queue에 넣는 `@Async`와 달리, 메시지 큐에는 다른 프로세스도 이해할 수 있는 데이터가 저장된다.

이 구조를 이루는 구성 요소는 다음과 같다.
...
다만 Producer가 `send()`를 호출했다는 사실만으로 메시지가 안전하게 보관됐다고 단정할 수는 없다. Broker가 메시지를 받았는지 확인하는 설정, 복제 수, 보존 정책이 요구 수준에 맞아야 한다. 메시지 큐를 사용하면 복구 수단을 설계할 수 있지만, 아무 설정 없이 자동으로 모든 메시지가 보장되는 것은 아니다.
...
여러 Outbox와 Connector로 나누면 처리량을 높이고 장애의 영향을 분리하는 데 도움이 될 수 있다. 하지만 같은 업무 흐름의 Event가 서로 다른 경로를 지나면 도착 순서를 맞추기 어려워질 수 있다. 같은 주문처럼 하나의 업무 단위에 속한 Event의 순서가 중요하다면 일관된 발행 경로와 Key 전략을 함께 설계해야 한다.

Outbox가 재발행할 수 있다는 것은 같은 Event가 두 번 전달될 수도 있다는 뜻이다. 중복 문제를 살펴보기 전에, 병렬 처리 과정에서 Event 순서가 어떻게 달라질 수 있는지 먼저 확인해 보자.
...
서로 다른 Partition은 서로 다른 Consumer가 동시에 처리할 수 있다. 따라서 `결제 완료`가 Partition 0에, 같은 주문의 `주문 취소`가 Partition 1에 들어가면 처리 시점이 뒤바뀔 수 있다. Kafka에 먼저 기록된 순서만 보고 Topic 전체의 처리 순서를 기대해서는 안 된다.
...
이제 같은 주문의 Event를 같은 Partition에서 순서대로 읽을 수 있게 됐다. 하지만 Consumer가 Event를 처리한 직후 장애가 발생하면 같은 Event를 다시 받을 수 있다.
...
중복에 대비했다고 해서 모든 실패를 무한정 재시도해도 되는 것은 아니다.
...
예를 들어 검색어 통계를 잠깐 늦게 기록하거나 다시 계산할 수 있다면 `@Async`만으로 충분할 수 있다. 반면 결제 완료 뒤 반드시 발급해야 하는 이용권처럼 프로세스 재시작 후에도 작업이 남아야 한다면, 처리 시간이 짧더라도 Broker를 검토할 이유가 있다. 선택 기준은 작업 시간보다 유실과 복구 요구에 가깝다.
...
**`@Async`와 메시지 큐는 처리 속도보다 작업을 어디에 보관하고 장애가 발생했을 때 어떻게 복구하는지가 다르다. 짧고 다시 만들 수 있는 부수 작업이라면 `@Async`가 단순한 선택일 수 있다. 반면 프로세스가 종료돼도 작업이 남아야 하거나 재시도·중복 처리·순서 보장·독립 확장이 필요하다면 메시지 큐를 검토할 이유가 커진다.**
