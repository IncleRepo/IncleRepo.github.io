...
비동기로 분리해도 이메일 전송에는 여전히 3초가 걸린다. 달라지는 것은 사용자가 기다리는 시간이다. 실제 처리 시간과 필요한 Network·외부 시스템 처리량은 그대로이며, 비동기는 **호출자가 작업 완료를 기다리지 않도록 실행 흐름을 분리하는 방식**이다.
...
Thread 전환은 Annotation 자체가 아니라 Spring이 연결한 비동기 실행 구조에서 일어난다. 그렇다면 현재 요청 Thread 대신 누가 이 메서드를 실행할까?
...
Spring의 `AsyncUncaughtExceptionHandler`는 처리되지 않은 예외를 기록하는 데 사용할 수 있다. 유실된 작업을 다시 실행하려면 로그·모니터링·실패 저장·재시도 정책을 별도로 마련해야 한다.
...
설정에 적힌 숫자는 작업을 즉시 실행할지, 메모리에서 기다리게 할지, 더는 받지 않을지를 결정하는 용량 제한이다. 실제로는 먼저 끝난 작업이 Queue의 작업을 가져갈 수 있어 숫자가 달라질 수 있지만, 작업을 받아들이는 순서는 이 원리로 이해할 수 있다.
...
실제로 늘어나는 Thread 수는 `corePoolSize`, Queue 용량과 `maxPoolSize`가 함께 결정한다. Queue가 너무 작으면 순간적인 요청 증가에도 Thread가 빠르게 늘고 Rejection이 발생할 수 있다.

Queue가 지나치게 크면 많은 작업이 오래 기다리면서 메모리를 계속 사용하고, 사용자가 응답을 받은 뒤 실제 이메일은 한참 늦게 발송될 수 있다. Queue 길이는 **담을 수 있는 작업 수와 허용할 수 있는 처리 지연**을 함께 고려해 정해야 한다.
...
메시지 큐와 `@Async`의 핵심 차이는 실행 Thread보다 작업을 보관하는 위치에 있다.
...
Kafka에는 이메일 메서드가 아니라 “주문이 완료됐다”는 사실을 약속된 데이터 형식으로 기록한다. 이처럼 업무에서 발생한 사실을 나타내는 메시지를 이 글에서는 **Event**라고 부른다.
...
그러나 속도 차이가 계속되면 밀린 메시지가 초당 500건씩 늘어난다. 메시지 큐는 처리할 작업을 보관하지만 Consumer의 처리 속도는 그대로다. 장기적으로는 Consumer의 처리량을 Producer의 생산량 이상으로 확보해야 한다.
...
`send()`는 메시지 전송을 시작하고, Broker가 메시지를 기록했는지는 비동기 결과를 통해 확인한다.

`acks`는 Producer가 몇 단계의 Broker 확인을 받은 뒤 전송 성공으로 판단할지를 정하는 설정이다. 복제는 같은 메시지를 여러 Broker에 나누어 보관해 일부 Broker에 장애가 생겨도 복구할 수 있게 하는 방식이다. `send()`를 호출한 시점과 Broker가 메시지를 안전하게 저장한 시점 사이에는 이러한 확인 과정이 있다.

보존 기간이 지나치게 짧으면 Consumer가 복구하기 전에 메시지가 사라질 수 있다. 복구 가능성은 메시지 큐 도입뿐 아니라 보존 기간과 Producer·Consumer의 처리 정책을 함께 설계했을 때 확보된다.
...
Kafka의 Exactly-Once 기능은 Kafka 안에서 메시지를 읽고 다시 쓰는 과정에 범위가 한정된다. 외부 이메일 발송이나 DB 변경 같은 Side Effect는 별도의 중복 방지와 정합성 처리가 필요하다.
...
서로 다른 주문 사이의 전역 순서를 맞출 필요가 없으므로 여러 Partition에서 병렬 처리할 수 있다. **순서를 지켜야 하는 Event의 범위를 Key로 표현해야** 같은 주문의 Event가 같은 Partition으로 들어간다.

재시도 횟수와 간격은 실패 원인과 업무가 허용하는 지연을 기준으로 제한해야 한다.
...
DLQ나 DLT는 실패한 메시지를 보관하는 장소다. 실패 원인을 확인하고 데이터를 수정하거나 장애를 해소한 뒤, 누가 어떤 기준으로 다시 처리할지 운영 절차를 마련해야 한다.
...
지금까지 살펴본 `@Async`와 메시지 큐의 핵심 차이는 실패했을 때 작업이 어디에 보관되고 어떤 방식으로 복구되는지에 있다.

## 선택하기 전에 필요한 처리 보장 수준을 확인한다

선택 기준은 작업의 속도나 애플리케이션 수보다 실패 복구와 전달 보장에 있다. 하나의 애플리케이션에서도 메시지 큐를 사용할 수 있고, 여러 서비스가 있어도 짧은 로컬 작업에는 Executor를 사용할 수 있다.
...
메시지 큐는 직접 HTTP 호출처럼 실행 시점에 서로 기다리는 결합을 줄이는 대신, Event 계약에 대한 결합을 만든다. Consumer는 Producer가 발행하는 Event의 이름·필드·의미에 의존하므로 Event에도 변경 규칙이 필요하다.
...
이 질문들은 비동기 구조를 선택하기 전에 빠뜨리지 말아야 할 판단 순서로 사용한다.
