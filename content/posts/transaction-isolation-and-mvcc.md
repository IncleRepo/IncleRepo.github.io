...
모든 트랜잭션을 한 번에 하나씩만 실행하면 이런 고민은 줄어든다. 하지만 요청이 100개 들어오면 99개는 앞의 요청이 끝나기를 기다려야 한다. 실제 데이터베이스는 가능한 한 여러 트랜잭션을 동시에 처리하면서도, 각 트랜잭션에 정해진 기준에 맞춰 데이터를 보여줘야 한다.
...
두 트랜잭션이 같은 데이터를 다룰 때, 조회 시점에 따라 어떤 결과가 나오는지 살펴보자.
...
격리 수준이 높아질수록 더 많은 이상 현상을 막는 대신 동시에 처리할 수 있는 범위가 줄거나 Lock 대기와 관리 비용이 늘 수 있다. 업무에서 허용할 수 없는 읽기 현상을 먼저 정하고 그에 맞는 수준을 선택해야 한다.
...
InnoDB는 행 전체의 복사본을 영구히 쌓아두는 대신 이전 값을 복원하는 데 필요한 정보를 관리한다. 더 이상 어떤 트랜잭션도 과거 버전을 필요로 하지 않으면 해당 정보는 정리할 수 있다.
...
이런 조회를 **일관된 읽기(Consistent Read)**라고 한다. InnoDB 문서의 Consistent Nonlocking Read가 이에 해당한다. 다른 트랜잭션이 값을 바꾸더라도 Read View가 정한 기준에 맞춰 결과를 반환한다.
...
Read View가 정한 읽기 기준을 Snapshot이라고도 표현한다.
...
두 번째 SQL도 데이터베이스에서 다시 실행된다. 이때 InnoDB가 A의 Read View에 맞는 과거 버전을 찾아 100을 반환하므로 처음과 같은 값이 보인다.

반면 Read Committed에서는 두 번째 일반 `SELECT`가 새로운 Read View를 만든다. B가 이미 Commit했다면 새로운 읽기 기준에는 B의 변경이 포함되므로 120을 볼 수 있다.

Repeatable Read에서는 이 읽기 기준이 트랜잭션 동안 유지된다. 그렇다면 트랜잭션을 오래 열어 두면 과거 버전도 계속 보관해야 할까?

## 트랜잭션이 길면 과거 버전도 오래 남는다
...
따라서 읽기 전용 트랜잭션도 필요 이상으로 오래 열어두면 Undo 정리를 늦추고 저장 공간과 조회 비용에 영향을 줄 수 있다. 변경 여부보다 과거 버전을 얼마나 오래 필요로 하는지가 중요하다.

지금까지는 과거 버전을 이용해 조회 결과를 유지하는 방법을 봤다. 그런데 읽은 값을 바로 수정해야 한다면, 그 사이에 다른 트랜잭션이 값을 바꾸는 상황도 다뤄야 한다.
...
Repeatable Read에서도 `SELECT ... FOR UPDATE`는 변경할 현재 행을 확보하기 위해 가장 최근 상태를 읽는다. 일반 Consistent Read는 읽기의 일관성이 목적이지만, Locking Read와 `UPDATE`, `DELETE`는 가장 최근 상태를 기준으로 잠금 대상을 찾는다.
...
MVCC는 일반 조회에서 매번 공유 Lock을 잡지 않고도 일관된 값을 보여준다. 현재 상태를 바꾸는 `UPDATE`, `DELETE`와 잠금 조회인 `SELECT ... FOR UPDATE`에는 Lock이 사용되므로, 읽기와 변경의 목적에 맞춰 두 방식이 함께 동작한다.
...
같은 Snapshot에서 여러 조회를 묶거나 지연 로딩을 사용해야 한다면 읽기 전용 트랜잭션이 유용하다. 조회 메서드마다 관성적으로 붙이기보다 필요한 경계를 의식해서 사용한다.
...
**MVCC는 일반 조회가 서로 기다리는 시간을 줄이면서 각 트랜잭션에 맞는 과거 버전을 보여준다. 현재 상태를 기준으로 변경하는 작업에는 Lock과 별도의 동시성 제어가 함께 사용된다.**
