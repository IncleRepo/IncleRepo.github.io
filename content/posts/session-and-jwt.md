...
slug = '11'
aliases = ['/posts/011/']
date = 2026-04-29T21:40:00+09:00
...
이후 요청을 로그인한 사용자와 연결하려면 클라이언트가 매 요청에 인증 정보를 보내야 한다. 서버는 이 정보를 바탕으로 다음 내용을 확인한다.
...
두 방식은 인증 상태를 두는 위치와 요청마다 검증·갱신·폐기하는 방법이 다르다.
...
Session ID는 서버에 저장된 상태를 찾는 임의의 식별자다. 공격자가 이 값을 알아내면 해당 사용자인 것처럼 요청할 수 있으므로 추측하기 어렵게 만들어야 한다.
...
### Cookie는 Session ID를 다음 요청에 실어 보낸다

세션 방식에서는 Session ID를 반복해서 보내기 위해 Cookie를 자주 사용한다. Session은 서버가 보관하는 사용자 상태이고, Cookie는 브라우저가 Session ID를 전달하는 수단이다.
...
일반적인 서버 세션 방식에서 Cookie에는 `SESSION=7f3a...` 같은 식별자가 들어간다. 서버가 이 식별자로 `userId`와 권한을 찾아 현재 요청에 연결한다.
...
이 속성들은 인증 정보가 노출되거나 원치 않는 요청에 자동으로 포함될 가능성을 줄인다. 각 속성이 막는 공격 경로가 다르므로 서비스의 요청 구조에 맞게 조합해야 한다.

{{< callout type="warning" title="HttpOnly가 줄이는 공격 경로" >}}
...
인증 상태를 공용 저장소에 모으는 만큼 요청마다 네트워크 조회가 추가되고, 저장소의 지연과 장애도 관리해야 한다.
...
여러 서버가 공용 Session Store를 사용하면 세션 방식도 확장할 수 있다. 이때는 상태를 둘 위치와 요청마다 지불할 네트워크 비용을 함께 고려해야 한다.
...
JWT는 JSON 정보를 Token으로 전달하는 표준이다. 여기서 다루는 `Header.Payload.Signature`는 인증에 흔히 사용하는 JWS 서명 형태로, 정보를 담는 부분과 이를 검증하는 부분이 나뉜다.
...
Payload는 전송하기 쉬운 문자열 형태로 바뀔 뿐 누구나 Decode해 내용을 읽을 수 있다. 포장 방법을 바꾼 것이지 자물쇠를 채운 것이 아니기 때문이다.
...
서명은 Header와 Payload가 발급 이후 바뀌지 않았는지, 서버가 신뢰하는 발급자가 만든 Token인지 확인한다. Payload의 기밀성은 별도의 암호화로 확보해야 한다.
...
10분 뒤 데이터베이스에서 사용자의 권한을 `ADMIN`으로 바꿔도 기존 Token에는 `role=MEMBER`가 그대로 남고 서명 검증도 통과한다. 별도의 재발급이나 현재 권한 조회가 없다면 만료 전까지 이전 권한 정보를 사용한다.
...
JWT를 사용해도 현재 권한이나 강제로 폐기한 Token인지 확인해야 한다면 외부 저장소를 조회할 수 있다. 저장소 조회 여부는 JWT 자체보다 인증 정책에 따라 달라진다.
...
API에 자주 보내는 인증 정보와 재발급할 때만 사용하는 인증 정보의 수명을 나누면 이 문제를 줄일 수 있다.

- **Access Token:** API 요청에 사용하는 비교적 짧은 수명의 인증 정보
- **Refresh Token:** 새 Access Token을 발급받기 위한 더 긴 수명의 인증 정보

이 역할 구분은 Token 형식과 별개다. Access Token은 서명된 JWT나 서버가 조회해 검증하는 임의 문자열로 만들 수 있고, Refresh Token도 서버 저장소에서 찾아 검증하는 임의 문자열로 만들 수 있다.
...
강제 폐기, 기기별 로그인과 재사용 탐지까지 제공하려면 이러한 서버 상태를 함께 관리한다. JWT로 매 요청의 인증 조회를 줄이는 것과 로그인 이력을 관리하는 것은 서로 다른 역할이다.
...
CSRF 방어는 프론트엔드와 API가 같은 사이트인지, 요청을 어떤 방식으로 보내는지와 브라우저 정책을 함께 확인해 구성한다. 위 방법은 현재 구조에 맞는 것을 조합해서 사용한다.
...
세션과 JWT 중 무엇을 선택할지는 인증 상태를 어디에 두고 얼마나 빠르게 변경·폐기해야 하는지에 따라 달라진다. 다음 질문으로 필요한 보장 수준을 정해 보자.
...
앞의 권한 변경 사례처럼 변경을 다음 요청부터 반영해야 한다면, 서버 상태를 갱신하거나 현재 권한을 다시 조회할 수 있어야 한다. Token 만료까지 기다려도 되는 서비스와는 검증 경로가 달라진다.
...
규모가 큰 환경에서도 세션은 공용 저장소로 여러 서버에서 사용할 수 있고, JWT도 폐기와 Refresh Token 관리 때문에 서버 상태를 가질 수 있다. 보안 수준은 형식보다 Token의 수명·저장·폐기와 공격 방어 정책에 따라 달라진다.
...
이 흐름은 인증을 설계할 때 빠뜨리지 말아야 할 질문의 순서로 사용한다.
...
세션은 클라이언트가 보낸 식별자로 서버의 인증 상태를 찾고, JWT는 인증에 필요한 사용자 정보 일부를 Token에 담아 각 서버가 검증한다. 실제 선택에서는 상태 위치와 함께 수명, 강제 폐기, 권한 변경, 재발급, 브라우저 저장 방식과 보안 요구까지 설계해야 한다.
