...
인덱스의 효과는 쿼리의 조건과 정렬 방식이 인덱스의 정렬 구조와 맞고, MySQL이 실제 실행 경로로 그 인덱스를 선택할 때 나타난다.
...
“이 쿼리 인덱스 타나요?”라는 질문은 MySQL이 인덱스를 조회 경로로 선택했는지 묻는 말이다. 성능을 판단하려면 여기에 한 가지를 더 확인해야 한다. 그 인덱스로 읽을 범위를 얼마나 줄였을까?
...
정렬된 Key로 시작 위치를 찾고 필요한 Leaf Page를 이어 읽기 때문에 단건 조회뿐 아니라 범위 조회와 정렬에도 유리하다.
...
반면 `created_at < ?`는 특정 값 하나가 아니라 일정 구간을 읽는 범위 조건이다. 복합 인덱스에서 범위 조건이 시작되면 뒤의 `id`까지 탐색 범위를 줄이는 데 사용하는 방식에는 제약이 생길 수 있다. `id`는 정렬이나 추가 조건 판단에는 도움을 줄 수 있지만, 앞의 `status`처럼 탐색할 연속 구간을 고정하지는 못한다.
...
복합 인덱스의 순서는 선택도만으로 결정하지 않는다. PUBLISHED 값이 많아 선택도가 낮더라도 `status = 'PUBLISHED'`로 먼저 영역을 고정해야 뒤의 날짜 정렬을 효율적으로 사용할 수 있다. `=` 조건으로 어느 컬럼까지 값을 고정하는지, 범위 조건이 어디서 시작되는지, `ORDER BY`와 `GROUP BY`가 어떤 순서인지, 실제 쿼리가 얼마나 자주 실행되는지를 함께 봐야 한다.
...
응답 컬럼을 추가할수록 인덱스도 넓어져 저장 공간과 쓰기 비용이 커지고, Page 하나에 담을 수 있는 Key는 줄어든다. 자주 실행되는 쿼리에서 실제 행 조회를 줄이는 이점이 이 비용보다 큰지 확인해 적용한다.
...
변경 후 `key`에 원하는 인덱스가 보이더라도 검증할 내용은 남는다. 실제로 읽는 범위가 충분히 줄었는지 이어서 확인해 보자.
...
트리 형태의 실행 계획이 익숙하지 않다면 MySQL Workbench의 Visual Explain으로 Join 순서와 테이블 접근 경로를 먼저 살펴볼 수 있다. 이후 `key`, `used_key_parts`, 예상·실제 행 수와 실행 시간을 함께 읽어 화면에 표시된 접근이 실제로 효율적인지 검증한다.

인덱스로 읽은 범위를 확인한 뒤에는 `Extra`에 표시되는 추가 작업도 해석해야 한다. `Using filesort`와 `Using temporary`는 쿼리 실패가 아니라 정렬이나 임시 결과 같은 추가 작업을 나타낸다.

## Using filesort와 Using temporary가 보이면 실제 비용을 확인한다

EXPLAIN의 `Extra`에 이런 문구가 나타나면 처리하는 행 수와 실행 빈도, 실제 시간을 함께 확인해 추가 작업의 비용을 판단한다.
...
표시의 유무보다 쿼리 조건과 인덱스 정렬이 어긋난 이유를 찾고, 변경 전후의 실행 계획과 시간을 비교해야 한다.
...
최종 반환 건수와 실제 읽는 행 수, 테이블 크기와 실행 시간을 비교하면 이 Full Scan이 합리적인 선택인지 판단할 수 있다.
...
각 인덱스가 읽는 행 수와 결과를 합치는 비용을 함께 보면 `index_merge`의 효율을 판단할 수 있다.
...
`OR`을 `UNION ALL`로 바꾸는 선택은 중복 허용 여부, 각 쿼리의 인덱스 효율, 결과를 합치는 비용과 실제 실행 시간으로 판단한다. `UNION ALL`은 중복을 제거하지 않으므로 두 조건을 모두 만족하는 행이 있으면 결과가 중복될 수 있다.

## 느린 쿼리를 확인하는 순서

인덱스 튜닝은 쿼리가 느린 이유를 예상하고 실행 계획과 측정 결과로 검증하는 반복 작업이다.
...
**좋은 인덱스 설계는 쿼리가 어떤 범위를 어떻게 탐색할지 이해하고, 필요한 인덱스만 만든 뒤 실행 계획과 실제 수행 시간으로 동작을 검증한다.**
