...
이 모델을 Java 코드로 옮기면 대상은 `Order`와 `OrderLine`, 상태는 `OrderStatus`, 행동과 규칙은 `Order.cancel()`으로 표현할 수 있다. 다만 무엇을 중요한 대상으로 보고 어떤 규칙을 담을지는 개발자가 혼자 정할 수 없다. 모델의 재료는 실제 업무를 아는 사람과의 대화에서 나온다.
...
도메인 전문가와 개발자가 함께 정한 공통 언어를 **유비쿼터스 언어**라고 한다. 우리말로는 보편 언어라고도 부른다. 회의에서 주문 취소라고 부른 개념은 화면과 문서, 테스트와 코드에서도 같은 의미로 이어져야 한다.
...
먼저 업무 담당자와 개발자가 `취소가 정확히 무엇인지` 합의해야 한다. `cancel()`은 그 합의가 코드에 드러난 결과다. 회의에서는 `주문 취소`라고 부르면서 코드에 `changeStatus(4)`만 남겨 두면, 개발자는 숫자 `4`의 의미를 다시 찾아야 한다. 함께 정한 언어가 문서에만 머물지 않고 코드까지 이어져야 하는 이유다.
...
이벤트 스토밍에서는 여러 사람이 알고 있던 사건과 규칙을 한곳에 펼쳐 놓는다. 개발자는 그 대화에서 중요한 대상과 규칙을 찾아 모델 후보로 정리한다. 포스트잇의 색이나 세부 진행 순서보다 서로 다른 이해를 드러내고 맞추는 대화가 핵심이다.
...
패키지와 모듈, 서비스는 경계를 구현하는 수단이므로 바운디드 컨텍스트와 반드시 일대일로 맞아야 하는 것은 아니다. 하나의 모놀리식 애플리케이션 안에도 여러 바운디드 컨텍스트를 둘 수 있다. 하나의 모델로 설명하기 어려워지는 의미의 경계가 보일 때 나누면 된다.
...
외부에서는 상태 값을 직접 바꾸지 않고 `cancel()`을 호출한다. 이 메서드는 Setter를 감춘 코드가 아니라, `취소`라는 업무 의도와 조건, 상태 변경을 한곳에 보여준다.
...
Repository는 테이블이 아니라 Aggregate 단위로 저장하고 조회한다. 예를 들어 `orders`와 `order_lines`가 서로 다른 테이블에 저장되더라도, 주문 상품은 `Order` Aggregate의 일부로 복원하고 Root를 통해 변경한다.
...
JPA 구현에서는 이미 영속 상태인 Entity의 변경이 Dirty Checking으로 반영되어 `save()` 호출이 필요하지 않을 수도 있다. 앞의 예제에서는 특정 JPA 동작보다 `Aggregate를 불러오고 변경한 뒤 저장한다`는 개념적인 흐름을 보여주기 위해 명시했다.
...
주문 취소 조건은 `Order` 안에 자연스럽게 들어갔다. 하지만 모든 업무 규칙에 주인이 이렇게 분명한 것은 아니다.
...
규칙을 어느 Entity나 Value Object에도 자연스럽게 둘 수 없고, 규칙 자체가 독립적인 업무 개념이라면 **Domain Service** 후보가 된다. 여기서는 회원 등급과 주문을 같은 판매 정책 컨텍스트에서 다룬다고 단순화하고, 두 정보를 함께 살펴 할인액을 계산하는 정책을 예로 들어 보자.
...
상품 정보를 조회하듯 데이터를 주고받을 수도 있지만, 주문 취소처럼 상태를 바꾸고 후속 처리를 이어 가야 할 때도 있다. 이런 흐름을 메시지로 표현할 때는 `해 달라는 요청`과 `이미 일어난 사실`을 구분한다. 전자가 **Command**, 후자가 **Domain Event**다.
...
Spring의 애플리케이션 이벤트는 같은 애플리케이션 안에서 Domain Event를 전달할 때 사용할 수 있다. Kafka 같은 메시지 브로커는 서로 다른 서비스나 컨텍스트에 Integration Event를 전달하는 수단이 될 수 있다. 어떤 기술을 쓰든 Domain Event와 외부 메시지 계약을 반드시 같은 클래스로 만들 필요는 없다.

Integration Event는 보통 주문 트랜잭션이 성공한 뒤 외부로 전달한다. 비동기 메시징을 선택하면 발행 실패와 재시도, 중복 처리, DB 트랜잭션과 메시지 사이의 정합성도 함께 설계해야 한다. 이벤트를 사용한다고 이 문제가 저절로 해결되지는 않는다.

### 외부 모델을 내 언어로 번역한다

컨텍스트 사이에 공개 계약을 두어도 각 컨텍스트가 사용하는 모델과 용어까지 같아지는 것은 아니다. 결제 컨텍스트가 `PaymentResponse`에 `paymentCode`와 `cancelAmount`를 담아 반환한다고 해보자. 이 객체를 주문 도메인까지 그대로 전달하면 주문 코드도 결제 컨텍스트의 이름과 구조를 알아야 한다.
...
정해진 패키지 구조를 복사하는 일이 아니라, 업무 담당자와 개발자가 모델을 함께 만들고 구현에서 발견한 내용을 다시 반영하는 과정이다. 분석을 모두 끝내고 완벽한 모델부터 만드는 순서도 아니다. 모델은 업무를 더 알아갈수록 함께 바뀐다.
...
DDD는 도메인 전문가와 개발자가 업무를 함께 이해하는 데서 출발한다. 같은 언어를 사용하고, 그 언어와 모델이 통하는 경계를 정한다. 경계 안에서는 중요한 상태와 행동, 규칙을 코드로 표현하고, 다른 컨텍스트와는 합의한 계약으로 협력한다.
...
DDD의 목적은 Entity, Aggregate와 Event를 많이 만드는 데 있지 않다. 복잡한 업무를 정확하게 이해하고, 그 이해가 코드에서도 같은 의미로 이어지게 만드는 데 있다. 용어를 외우는 일보다 표에 적은 질문을 실제 업무에 던져 보는 일이 먼저다.
