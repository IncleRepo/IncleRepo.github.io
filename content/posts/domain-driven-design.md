...
DDD의 Context Map에는 컨텍스트 사이의 관계를 표현하는 여러 패턴이 있다. 기능과 정보를 제공하는 쪽을 **업스트림**, 이를 사용하는 쪽을 **다운스트림**이라고 한다. 다운스트림은 두 모델을 얼마나 독립적으로 유지할지에 따라 관계를 선택한다.
...
하나의 업스트림을 여러 컨텍스트가 이용한다면 공통으로 사용할 서비스와 데이터 형식이 필요하다. 이때 다음 두 패턴을 함께 사용할 수 있다.
...
이번 예에서는 상품 컨텍스트가 상품 정보를 공개된 계약으로 제공하고, 주문 컨텍스트가 이를 자신의 모델로 번역한다. 업스트림은 공개 호스트 서비스와 공표된 언어를 제공하고, 다운스트림은 부패 방지 계층으로 자신의 모델을 지키는 관계다.
