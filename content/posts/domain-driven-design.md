...
Context Map의 관계 패턴은 두 팀이 모델과 변경의 영향을 어떻게 나눌지 설명한다. 두 팀이 일부 모델을 대등하게 공동 소유할 수도 있고, 한쪽이 제공한 모델을 다른 쪽이 받아들이는 방향이 생길 수도 있다. 여기서는 이 관계를 두 가지 질문으로 나눠 보자.
...
공유 커널은 두 팀이 대등하게 공동 소유하는 관계다. 순응자와 부패 방지 계층처럼 한쪽이 모델을 제공하고 다른 쪽이 이를 사용한다면 제공자는 **업스트림**, 사용자는 **다운스트림**이 된다. 이런 방향이 정해졌다면 다음으로 업스트림이 무엇을 공개할지 정한다.
