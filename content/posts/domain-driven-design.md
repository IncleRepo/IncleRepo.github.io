...
합의한 용어는 코드에서도 그대로 이어져야 한다. 회의에서는 `주문 취소`라고 부르면서 코드에 `changeStatus(4)`만 남겨 두면, 개발자는 숫자 `4`가 취소를 뜻하는지 다시 찾아야 한다. 용어집과 코드가 서로 다른 말을 사용하면 함께 정한 의미도 구현 과정에서 끊어진다.
...
이벤트 스토밍이 Entity나 Aggregate를 자동으로 찾아주는 공식은 아니다. 여러 사람이 자신이 알고 있던 사건과 규칙을 한곳에 펼쳐 놓고 이야기하면, 개발자는 그 안에서 중요한 대상과 규칙을 찾아 모델 후보로 정리한다. 지금은 포스트잇의 색이나 세부 진행 규칙보다 이 과정에 집중하면 된다.
...
상품 정보를 조회하듯 데이터를 주고받을 수도 있지만, 주문 취소처럼 상태를 바꾸고 후속 처리를 일으키는 협력도 있다. 이런 업무 흐름을 메시지로 표현할 때는 작업 요청과 이미 일어난 사실을 구분할 수 있다. 전자가 **Command**, 후자가 **Domain Event**다.
