...
[레이어드 아키텍처](/posts/14/)에서는 코드의 책임과 의존 방향을 나눴다. 이번에는 그 코드에 담을 업무 자체를 살펴보려 한다. **우리가 같은 업무를 정말 같은 의미로 이해하고 있는가?** DDD는 이 질문에서 출발해, 함께 이해한 업무를 모델과 코드로 표현하는 방법을 다룬다.

## 같은 주문 취소를 다르게 이해할 때

주문팀, 결제팀, 물류팀과 고객센터가 따로 있는 쇼핑몰 회사를 생각해 보자.
...
업무에서 현재 문제를 푸는 데 필요한 대상과 상태, 행동과 규칙을 추려 표현한 것이 **도메인 모델**이다. 여기에 담기는 `배송이 시작된 주문은 취소할 수 없다` 같은 판단을 **업무 규칙** 또는 **도메인 로직**이라고 한다.
...
회의에서 합의한 주문 취소, 환불 요청과 출고 중단이 각각 코드의 행동으로 이어진다. 개발자도 `cancel()`의 조건을 설명하면서 업무 담당자와 같은 의미의 취소를 이야기할 수 있어야 한다.
...
서브도메인과 바운디드 컨텍스트를 나누고 컨텍스트 사이의 관계를 정하는 관점을 **전략적 설계**라고 한다. 큰 경계를 정했다면 그 안의 모델을 구체화할 차례다.

Entity, Value Object와 Aggregate로 모델을 만들고, Domain Service, Repository와 Domain Event로 규칙과 저장, 중요한 변화를 표현하는 관점은 **전술적 설계**라고 한다. 두 관점 모두 업무 대화에서 합의한 유비쿼터스 언어를 바탕으로 삼는다.
...
주문 컨텍스트 안에서는 주문과 금액을 어떻게 표현하고, 어떤 객체를 통해 상태를 바꿀지 정해야 한다. 이를 코드로 옮기며 전술적 설계의 개념들을 살펴보자.
...
주문 취소 조건은 `Order`에, 금액 계산은 `Money`에 두었듯이 업무 규칙은 그 규칙을 가장 잘 아는 Entity나 Value Object가 맡는다.

할인 정책처럼 한 객체에 맡기기 어려운 규칙도 있다. 여러 정보를 함께 살피는 규칙 자체가 독립적인 도메인 개념이라면 **Domain Service**로 표현할 수 있다.
...
이렇게 모델에 규칙을 담아도 실제 요청을 처리하려면 필요한 객체를 불러오고 그 행동을 호출해야 한다. 주문 취소 기능으로 돌아가 이 실행 흐름을 연결해 보자.
...
이처럼 Command와 Query에 서로 다른 모델과 처리 경로를 두는 방식을 **CQRS**라고 한다. 가장 단순한 형태에서는 하나의 애플리케이션과 데이터베이스를 그대로 사용하면서 코드의 읽기와 쓰기 경로만 나눌 수 있다. 규모와 요구사항에 따라 조회 전용 저장소와 변경 전용 저장소까지 분리해 각 경로를 독립적으로 확장할 수도 있다.
...
Context Map의 관계 패턴은 두 팀이 모델과 변경의 영향을 어떻게 나눌지 설명한다. 일부 모델을 함께 소유할 수도 있고, 한쪽이 제공한 모델을 다른 쪽이 받아들일 수도 있다. 후자의 경우 제공하는 쪽을 **업스트림**, 사용하는 쪽을 **다운스트림**이라고 한다.

이 관계를 두 가지 질문으로 나눠 보자.

**1. 두 팀은 모델과 변경의 영향을 어떻게 나눌까?**

- **공유 커널(Shared Kernel) — 같이 쓰되, 작게 쓴다.** 두 팀에 정말 같은 의미를 가진 모델과 코드만 대등하게 공동 관리한다. 어느 한쪽이 업스트림이 되는 관계가 아니며, 이를 바꿀 때도 함께 합의한다.
...
모델을 받아들일 방식을 정했다면, 업스트림이 제공할 기능과 데이터 형식도 함께 살펴봐야 한다.
...
관계 패턴과 공개 방식은 함께 조합할 수 있다. 이제 상품과 주문 컨텍스트의 관계에 지금까지의 선택을 적용해 보자.

### 관계와 계약을 주문 예제에 적용한다

상품 컨텍스트는 공개 호스트 서비스와 공표된 언어를 통해 현재 상품 정보를 `ProductInfo`라는 계약으로 제공한다. 주문 컨텍스트는 상품 모델을 그대로 따르는 대신 부패 방지 계층을 두고, 이 값을 주문 당시의 상품 정보인 `OrderedProduct`로 바꿔 저장한다.
...
주문 취소가 환불과 쿠폰 복구로 이어지려면 취소 요청과 처리 결과를 구분해 전달해야 한다. 각 메시지가 맡는 역할은 다음과 같다.
...
**Context Map은 각 경계에서 선택한 관계와 계약을 기록한 지도다.** 이 그림을 함께 보며 상품 정보가 바뀌었을 때 어느 경계를 확인할지, 주문 취소 결과를 어느 컨텍스트가 받는지 이야기할 수 있다.
...
### 컨텍스트 사이의 관계와 계약을 정할 때

| 개념 | 답하려는 질문 |
|---|---|
| Shared Kernel | 어느 모델과 코드를 두 팀이 함께 관리할까? |
| Conformist | 업스트림의 모델과 언어를 그대로 따를 수 있는가? |
| Anti-Corruption Layer | 외부 모델을 내 컨텍스트의 언어로 어떻게 바꿀까? |
| Separate Ways | 통합 비용을 감수할 만큼 협력이 필요한가? |
| Open Host Service | 업스트림은 어떤 공식 출입구를 제공할까? |
| Published Language | 경계에서 어떤 데이터 형식과 의미를 함께 사용할까? |
| Integration Event | 확정된 사실을 외부에 어떤 계약으로 공개할까? |
