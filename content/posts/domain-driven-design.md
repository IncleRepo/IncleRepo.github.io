...
앞에서 주문은 상태를 가지며, 배송이 시작되면 취소할 수 없다는 규칙을 찾았다. 이를 다음처럼 상태 값만 바꾸는 코드로 구현할 수 있다.

```java
order.setStatus(OrderStatus.CANCELED);
```

상태를 직접 바꾸면 취소 조건이 코드에 드러나지 않아 호출하는 곳마다 규칙을 확인해야 한다. 이 규칙을 `Order`가 책임지게 하려면 주문 모델을 이루는 요소부터 구분해야 한다.

주문처럼 계속 추적할 대상은 Entity, 금액과 주소처럼 값 자체가 중요한 것은 Value Object로 표현한다. 주문과 주문 상품이 함께 지켜야 할 변경 규칙은 Aggregate 경계로 묶는다.
