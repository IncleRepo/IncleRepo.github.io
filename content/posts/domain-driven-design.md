...
DDD가 이런 문제를 자동으로 해결해 주는 것은 아니다. DDD는 복잡한 업무를 사람과 코드가 같은 모델로 이해할 수 있도록, 업무를 배우고 모델을 계속 다듬어 가는 접근이다. 그러려면 먼저 우리가 해결하려는 업무가 무엇인지부터 알아야 한다.
...
다음 그림에서는 현실의 업무가 곧바로 코드가 되는 것이 아니라, 중요한 개념과 규칙을 고르는 과정이 사이에 있다는 점을 보면 된다.
...
단순한 순서처럼 보여도 질문은 계속 생긴다. 환불 요청이 실패하면 주문 취소도 되돌려야 할까? 이미 출고됐다면 취소가 아니라 반품으로 바뀌어야 할까? 이런 질문에 답하면서 모델에 들어갈 상태와 규칙이 드러난다.
...
도메인 전문가와 개발자가 같은 개념을 같은 의미로 사용하는 공통 언어를 **유비쿼터스 언어**라고 한다. 우리말로는 보편 언어라고도 부른다. 합의한 언어는 회의와 문서에만 머물지 않고 화면, 테스트와 코드까지 이어진다.
...
이 대화는 사건 목록보다 더 많은 재료를 남긴다. 사건을 일으키는 대상과 그 과정에서 지켜야 할 규칙도 함께 드러난다.
...
아직 `Order`가 어떤 객체이고 어디까지 묶여야 하는지는 정하지 않았다. 지금은 업무 대화가 코드의 이름과 규칙으로 이어질 재료를 만든다는 점만 기억하면 된다. Entity와 Aggregate를 살펴보면서 이 재료가 실제 코드로 바뀌는 과정을 이어 간다.
...
| 구분 | 던지는 질문 | 바라보는 공간 |
|---|---|---|
| 서브도메인 | 현실의 업무를 어떻게 나눌 것인가? | 문제 공간 |
| 바운디드 컨텍스트 | 그 업무를 표현하는 모델의 경계를 어디까지 둘 것인가? | 해결 공간 |

현실의 주문 서브도메인을 하나의 주문 컨텍스트가 맡을 수 있다. 시스템이 커지면 주문 접수와 주문 이력을 서로 다른 컨텍스트가 나누어 맡을 수도 있다. 둘을 반드시 일대일로 맞추는 것이 목적은 아니다.
...
바운디드 컨텍스트는 본질적으로 패키지나 배포 단위가 아니라 **모델과 언어의 의미 경계**다. 구현할 때 `order`, `payment` 같은 패키지나 Gradle 모듈로 드러낼 수 있고, 필요하다면 독립 서비스로 분리할 수도 있다. 그러나 폴더 하나를 만들었다고 저절로 바운디드 컨텍스트가 생기는 것은 아니다.

다음 그림에서는 큰 Context 경계와 그 안의 작은 Aggregate 경계를 구분해서 보면 된다.

![주문 컨텍스트 안의 여러 애그리거트와 상품 컨텍스트의 관계](/images/posts/domain-driven-design/context-and-aggregate.svg "바운디드 컨텍스트와 애그리거트 경계의 차이")

하나의 바운디드 컨텍스트 안에는 여러 Aggregate가 존재할 수 있다. 같은 Context 안에서도 각 Aggregate의 일관성 경계는 지켜야 하며, Context를 넘을 때는 상대방의 Entity를 내 모델처럼 직접 사용하지 않는 편이 좋다.
...
여기까지처럼 큰 업무와 모델의 경계를 찾는 일을 DDD의 **전략적 설계**라고 부른다. 이제 각 Context 안에서 Entity, Value Object와 Aggregate로 모델을 코드에 표현하는 **전술적 설계**로 내려가 보자.
...
외부 코드가 `OrderLine`을 직접 꺼내 수량만 바꾸면 총금액 재계산을 놓칠 수 있다. 그래서 관련 객체를 모으는 데서 그치지 않고 변경이 통과할 대표 객체를 정한다.
...
다음 그림에서는 `Order`가 내부 Entity와 Value Object를 소유한다는 점보다, 외부 변경이 Root를 통해 들어와 주문 규칙을 함께 지킨다는 점을 보면 된다.

![Order가 Aggregate Root가 되어 주문 상품과 배송지 변경을 통제하는 구조](/images/posts/domain-driven-design/order-aggregate.svg "주문 Aggregate의 구조")

`함께 일관성을 지킨다`는 말이 모든 값을 매번 동시에 바꾼다는 뜻은 아니다. 회원의 이름과 주소를 서로 다른 날에 바꿔도 같은 Member Aggregate일 수 있다. 중요한 것은 Member Root가 그 상태들의 유효성과 생명주기를 자신의 경계 안에서 책임진다는 점이다.
...
이제 `Order`는 스스로 취소 가능 여부를 판단할 수 있다. 하지만 HTTP 요청을 받고 Aggregate를 불러와 주문 취소라는 사용 사례를 끝까지 진행할 코드는 여전히 필요하다.

## Application은 도메인 모델로 사용 사례를 완성한다
...
Application Service는 이 기능을 완료하려면 무엇을 어떤 순서로 실행할지 조정하고, Domain은 업무적으로 무엇이 가능한지 판단한다. 규칙을 `Order`에 두면 관리자 기능이나 배치가 주문을 취소하더라도 같은 조건을 재사용할 수 있다.

### Repository는 Aggregate를 저장하고 복원한다

Order Aggregate를 하나의 일관성 단위로 정했다면 저장소에서 다시 불러올 때도 완전한 Order Aggregate로 복원하는 편이 자연스럽다. 이 계약을 **Repository**가 맡는다.
...
DDD의 Repository 개념과 Spring Data의 `JpaRepository`도 같은 것은 아니다. `OrderRepository`는 주문을 저장하고 복원한다는 도메인 관점의 계약이다. JPA와 MySQL로 이를 구현하는 일은 Infrastructure의 선택이 될 수 있다.
...
이 구분을 사용한다고 도메인 객체와 JPA Entity를 반드시 두 벌로 만들어야 하는 것은 아니다. 규모와 복잡성에 따라 하나의 클래스를 함께 사용할 수도 있다. 중요한 것은 저장 기술 때문에 업무 규칙을 표현하기 어려워지는지 살펴보는 일이다.
...
모든 업무 규칙을 Aggregate Root에 넣는 것도 정답은 아니다. 회원 등급과 주문 금액을 함께 봐야 하는 할인 정책처럼 어느 Aggregate 하나의 상태만으로 설명하기 어려운 독립적인 규칙이 있다.

먼저 해당 규칙을 한 Aggregate나 Value Object가 자연스럽게 맡을 수 있는지 확인한다. 어느 곳에도 무리 없이 들어가지 않고 규칙 자체가 중요한 도메인 개념이라면 **Domain Service** 후보가 된다.
...
간단히 말하면 Application Service는 진행과 조정을, Domain Service는 업무 판단과 계산을 맡는다. 다만 이름만 보고 기계적으로 나누는 공식은 아니다. 규칙을 가장 자연스럽게 설명할 책임이 어디에 있는지를 먼저 본다.

`DiscountPolicy`는 Domain Model에 속하지만 Order나 Member Aggregate의 구성원은 아니다. Domain Service도 도메인 개념이지만 Aggregate 경계 바깥에 존재할 수 있다.

이제 한 Context 안에서 모델을 만들고 사용하는 흐름은 갖춰졌다. 다음 문제는 주문 취소 이후 환불과 출고 중단처럼 다른 Context와 협력하는 방법이다.

## 다른 Context와 명시적으로 협력한다

바운디드 컨텍스트를 나눴다고 서로의 협력이 사라지는 것은 아니다. 대신 무엇을 주고받을지 계약으로 드러내야 한다.
...
대신 API 응답, 명시적인 DTO나 Event를 통해 필요한 정보만 받고 주문 컨텍스트의 언어로 바꾼다.
...
Event라고 반드시 Kafka나 비동기 처리가 필요한 것은 아니다. 같은 애플리케이션 안에서 Spring Event로 동기 전달할 수도 있다. Kafka는 사건을 전달하는 기술 중 하나다. 메시지 큐를 사용한다면 이벤트 유실과 중복, DB 트랜잭션과의 불일치도 별도로 설계해야 한다.
...
이처럼 외부 모델을 내 Context의 언어로 번역해 내부 모델이 끌려가지 않도록 보호하는 경계를 **부패 방지 계층**이라고 한다. 영어로는 Anti-Corruption Layer, 줄여서 ACL이라고 부른다.

Context 사이의 관계를 한눈에 정리한 그림은 Context Map이라고 한다. 처음부터 모든 관계 패턴을 외울 필요는 없다. 어느 Context가 정보를 제공하고 누가 사용하는지, 어떤 계약으로 연결되는지부터 명확히 하면 된다.
...
DDD를 적용했다고 MSA나 CQRS를 함께 도입해야 하는 것은 아니다. 모놀리식 애플리케이션에서도 DDD를 적용할 수 있고, 평범한 레이어드 구조에서도 도메인 모델을 사용할 수 있다.

실제 팀도 특정 회사의 패키지 구조를 그대로 복사하는 것보다 다음 과정을 반복한다.
...
국내 DDD 적용 사례에서도 구현 패턴 하나보다 업무 담당자와 개발자가 모델과 경계를 함께 정하고, 그 결과를 코드로 이어 간 과정이 핵심으로 드러난다.
...
한 시스템 안에서도 복잡한 주문 정책에는 Rich Domain Model과 Aggregate를 사용하고, 단순 조회에는 Projection을 사용할 수 있다. 다른 Context와 느슨하게 협력해야 할 때만 Event를 검토할 수도 있다. 핵심 서브도메인에는 깊게 적용하고 일반 서브도메인은 단순하게 구현하는 식으로 투자 수준을 달리할 수 있다.
...
↓
Context 밖과는 명시적인 계약으로 협력한다
```

Entity는 식별성이 중요한 도메인 객체이고, Value Object는 값 자체와 의미가 중요한 도메인 객체다. Aggregate는 이들과 나란한 객체 종류가 아니라 Root가 내부 상태의 일관성을 책임지는 경계다.

Application Service는 도메인 객체를 준비하고 호출해 사용 사례를 진행한다. Domain Service는 어느 Aggregate에도 자연스럽게 속하지 않는 독립적인 업무 규칙을 표현할 때 후보가 된다. Repository는 테이블이 아니라 Aggregate를 저장하고 복원한다는 계약으로 바라본다.
