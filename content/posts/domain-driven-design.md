...
lastmod = 2026-08-28T20:20:00+09:00
draft = false
...
이렇게 업무에서 중요한 대상과 상태, 행동과 규칙을 추려 표현한 것이 **도메인 모델**이다.
...
`Order.cancel()`은 주문이 스스로 판단해야 할 규칙을 표현한다. 주문을 조회하고 이 행동을 실행한 뒤 저장하는 전체 흐름은 별도의 응용 서비스가 조정할 수 있다. 대화가 곧바로 클래스가 되는 것이 아니라, 대화에서 찾은 대상과 행동, 규칙을 모델로 정리한 결과가 코드에 나타난다.
...
바운디드 컨텍스트 하나에도 서로 다른 규칙을 맡는 객체 묶음이 여러 개 들어갈 수 있다. 뒤에서 살펴볼 **Aggregate**는 컨텍스트 안에서 함께 상태를 바꾸고 규칙을 지킬 더 작은 경계다. 지금은 바운디드 컨텍스트가 Aggregate보다 큰 모델의 범위라는 점만 확인하자.
...
업무를 여러 서브도메인으로 나누었다고 해서 모든 영역에 같은 시간과 비용을 쓸 필요는 없다. 회사에서 차지하는 중요도에 따라 핵심, 지원과 일반 서브도메인으로 구분하기도 한다.
...
경계를 정했다면 그 안에서 사용하는 모델을 코드로 옮길 차례다. Entity, Value Object와 Aggregate로 대상과 값, 변경 규칙을 표현하는 과정을 **전술적 설계**라고 한다.
...
회원 등급과 주문 금액을 함께 사용하는 할인처럼 여러 Aggregate를 살펴야 판단할 수 있는 규칙도 있다.
...
컨텍스트는 합의한 형식으로 정보를 주고받는다. 이때 전달하는 메시지는 상대에게 작업을 요청하는 것과 이미 일어난 사실을 알리는 것으로 나눌 수 있다. 전자가 **Command**, 후자가 **Domain Event**다.
