...
상품 상세 조회 요청을 받으면 애플리케이션은 데이터베이스에서 상품과 판매자 정보를 조회한다. 여러 테이블의 결과를 합치고 필요한 값을 계산한 뒤, API 응답에 맞는 DTO로 변환한다.
...
실제 서비스에 적용하려면 결과를 어디에 저장하고 얼마나 오래 사용할지 정해야 한다. 반복 조회의 비용에서 출발해 Redis의 저장 방식과 값의 수명, 동시 변경과 장애 후 복구까지 차례로 살펴보자.
...
같은 SQL과 Join을 반복하지 않으면 DB Connection을 점유하는 시간과 결과를 조립하는 작업도 줄어든다. **캐시는 이미 만든 결과를 재사용해 이 반복 비용을 줄인다.**

### 값이 있을 때와 없을 때

캐시에 원하는 값이 있어 바로 반환하는 경우를 **Cache Hit**라고 한다. 값이 없어 원본 저장소를 조회하는 경우는 **Cache Miss**다.
...
Cache Miss가 발생하면 Redis와 DB를 모두 방문한다. 반복 조회가 거의 없다면 Redis 왕복만 하나 더 추가된다. 캐시의 성능 이점은 같은 데이터를 반복해서 조회해 Hit가 쌓일 때 나타난다.
...
Buffer Pool은 애플리케이션의 최종 응답이 아니라 데이터베이스의 Page를 보관한다. 필요한 행이 메모리에 있더라도 SQL의 Join과 집계, 결과 전송과 DTO 변환은 다시 수행할 수 있다.
...
Buffer Pool은 DB 내부의 Page 접근을, Redis 캐시는 애플리케이션의 결과 생성 과정을 줄이는 데 각각 도움을 준다.
...
Redis는 애플리케이션과 별도로 실행되는 서버다. 여러 애플리케이션이 네트워크로 명령을 보내 같은 저장소의 값을 읽고 쓸 수 있다.

![메모리 기반 데이터 저장소 Redis](/images/posts/redis-cache-basics/legacy-01.png "Redis")

요청을 받은 Redis Server가 명령을 처리하고 결과를 돌려주는 흐름은 다음과 같다.
...
Local Cache와 Redis는 여러 서버가 값을 공유해야 하는지와 새로운 네트워크·장애 비용을 감수할 수 있는지를 기준으로 선택한다.
...
TTL을 정할 때는 오래된 값을 얼마 동안 허용할 수 있는지와 원본을 다시 조회하는 비용을 함께 본다.
...
Pipeline은 명령마다 응답을 기다리는 비용을 줄인다. 명령 사이에 다른 Client의 작업이 끼어들 수 있으므로, 함께 실행해야 하는 연산에는 단일 명령, Transaction이나 Lua Script가 필요하다.
...
Pipeline은 세 명령을 묶어 보내 네트워크 왕복을 줄이지만, 명령 사이에 다른 요청이 끼어드는 것은 막지 못한다. 명령 조합의 원자성은 별도의 방법으로 확보해야 한다.
...
장애 후 복구 방법은 Redis에 보관한 데이터의 성격에 따라 달라진다.
...
Redis와 MySQL의 책임은 다음 질문에 답하며 정한다.
...
`@Cacheable`은 Redis가 아니라 Spring Cache의 Annotation이다.
...
TTL, Key Prefix, 직렬화 방식과 Redis 장애 시 동작은 Provider와 `CacheManager` 설정에서 별도로 결정한다.
...
`beforeInvocation`의 기본값은 `false`다. 따라서 위 예제의 Cache 제거는 메서드가 예외 없이 끝난 뒤 실행된다. **메서드가 정상적으로 끝난 시점과 DB Transaction이 Commit된 시점은 서로 다를 수 있다.** Cache 제거를 Commit 성공과 반드시 묶어야 한다면 `@TransactionalEventListener`처럼 Commit 단계에 동작을 연결하는 방법을 검토해야 한다. 이 Annotation은 기본적으로 `AFTER_COMMIT` 단계에 Listener를 실행한다.
...
캐시 Annotation이 적용되는 시점은 Spring AOP의 호출 방식과 연결된다. 기본 Proxy 방식에서는 외부 Bean이 메서드를 호출할 때 실제 객체 앞의 Proxy가 먼저 Cache를 확인한다.
...
Redis Cache에 완성된 결과를 보관하면 반복되는 SQL과 Join, 결과 전송과 DTO 변환을 줄일 수 있다. 이 이점을 얻으려면 값의 수명과 변경, 장애 복구를 함께 관리해야 한다.
...
Redis를 도입할 때는 반복되는 작업을 어디까지 재사용할지, 값의 수명과 정합성을 어떻게 관리할지, 장애가 나도 복구할 수 있는 데이터인지까지 포함해 저장소의 책임을 설계해야 한다.
