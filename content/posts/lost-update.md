...
**두 요청 모두 성공했는데도 한쪽의 변경이 사라진다는 점** 때문에 발견하기 어려운 문제다. JPA에서도 값을 읽고 Java에서 수정한 뒤 다시 저장하면 같은 상황이 생길 수 있다.
...
조건 확인과 변경을 데이터베이스의 한 연산으로 처리하는 방식을 원자적 UPDATE라고 한다.
...
충돌로 실패한 요청에는 최신 상태를 알려주거나 작업을 다시 시도하는 등 후속 처리가 필요하다.
...
충돌이 잦아 같은 계산을 반복하고 있다면, 조회할 때부터 다른 요청의 변경을 기다리게 하는 방법을 검토할 수 있다.
...
“한 사용자는 같은 이벤트에 한 번만 참여할 수 있다”는 규칙을 생각해보자. 이번에는 이미 있는 값을 바꾸는 대신 새 참여 기록의 중복을 막아야 한다.
