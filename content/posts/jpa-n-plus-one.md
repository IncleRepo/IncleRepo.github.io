...
이것이 N+1 문제다. 처음 쿼리에서 Team을 함께 가져오지 않은 상태에서 `getName()`처럼 Team의 실제 값을 사용하면 추가 쿼리가 발생한다.
...
EAGER는 연관된 Team을 즉시 사용할 수 있게 준비하라는 Fetch 전략이다. Hibernate는 하나의 `JOIN` SQL 대신 Member를 먼저 조회하고 Team을 채우기 위한 `SELECT`를 추가로 실행할 수도 있다. 이 과정에서도 N+1이 발생할 수 있다.
...
N+1은 처음 쿼리에서 가져오지 않은 연관 데이터를 나중에 사용하면서 추가 쿼리가 반복되는 문제다. JPA 자체의 속도보다 조회 시점과 연관 데이터 사용 시점이 갈라진 구조에서 원인을 찾아야 한다.
