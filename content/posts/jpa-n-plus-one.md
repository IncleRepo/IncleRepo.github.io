...
Java 코드에는 추가 쿼리가 보이지 않아 원인을 찾기 어려울 수 있다. 평범한 Getter 호출이 연관된 Entity를 조회하는 SQL로 이어지기 때문이다.
...
목록을 조회한 뒤 연관 데이터를 사용하는 과정에서 추가 쿼리가 반복되는 것이 N+1 문제다.

추가 쿼리 수는 서로 다른 Team의 수와 영속성 컨텍스트의 상태에 따라 달라진다. 여러 Member가 같은 Team을 참조한다면 Hibernate가 앞에서 이미 조회한 Team을 다시 사용할 수 있다.
...
Team을 사용할 때마다 추가 조회가 발생한다면, 처음부터 Team을 준비해 두는 방법을 떠올릴 수 있다.

## EAGER로 바꾸면 해결될까

JPA의 `FetchType.EAGER`는 연관된 Team을 즉시 사용할 수 있게 준비하라는 Fetch 전략이다. Hibernate는 하나의 `JOIN` SQL 대신 Member를 먼저 조회하고 Team을 채우기 위한 `SELECT`를 추가로 실행할 수도 있다. 이 과정에서도 N+1이 발생할 수 있다.
...
일대다 Fetch Join은 결과 규모가 작고 Member 목록 전체가 꼭 필요할 때 유용하다. 같은 Team 데이터가 반복되므로 Member가 많아질수록 데이터베이스가 읽고 애플리케이션으로 전송하는 행 수도 크게 증가한다.
...
목표는 SQL을 한 번만 실행하는 것이 아니라 조회 전체 비용을 줄이는 데 있다. 현재 조회가 Entity를 필요로 하는지, 연관 관계의 모양과 페이징 여부가 어떤지에 따라 적절한 방법이 달라진다.
...
Batch 크기를 지나치게 키우면 `IN` 절과 한 번에 읽는 데이터가 커진다. 실제 목록 크기와 데이터베이스의 실행 계획을 기준으로 크기를 정해야 한다.
...
EntityGraph로 일대다 관계를 함께 가져오면 Fetch Join처럼 같은 Team 데이터가 반복될 수 있다. 따라서 일대다 관계와 페이징을 함께 사용할 때는 결과 행 증가를 똑같이 확인해야 한다.
...
Batch 환경에서도 조회 원리는 같다. 페이징 설정이 Reader 안에 들어 있을 뿐이다. Team ID를 먼저 일정 크기로 조회하고, 두 번째 쿼리에서 `WHERE team.id IN (...)`으로 필요한 연관 데이터를 가져오는 방법을 검토할 수 있다.

### distinct가 제거하는 중복의 범위를 확인한다

일대다 Fetch Join에 `distinct`를 붙이면 Hibernate가 같은 Team Entity를 하나로 합치는 데 도움을 줄 수 있다. 데이터베이스의 Join 단계에서는 여전히 Member 수만큼 행이 만들어질 수 있다.
...
N+1을 발견하면 다음 질문으로 조회 조건을 확인한 뒤 EAGER, Fetch Join, Batch Fetching과 DTO 조회 중 적합한 방법을 고른다.
...
**N+1 해결의 목적은 조회 전체 비용을 줄이는 데 있다. 쿼리 수뿐 아니라 데이터베이스가 읽는 행 수, 조회하는 Column과 데이터 양, 페이징의 안정성을 함께 살펴야 한다.**
