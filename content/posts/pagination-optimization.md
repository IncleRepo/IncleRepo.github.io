...
**Slice와 Keyset Pagination은 서로 다른 선택이다.** `Slice`에 `Pageable`을 넘기면 Count 쿼리를 생략한 Offset 방식일 수 있다. Slice는 응답에 무엇을 담는지 정하고, Offset과 Keyset은 데이터베이스에서 다음 위치를 어떻게 찾을지 정한다.
...
Keyset은 정렬과 맞는 인덱스, 올바른 Cursor 조건, 페이지 번호 직접 이동이 필요하지 않은 화면에서 장점을 얻는다. 데이터가 작고 임의 이동이 중요한 화면에서는 Offset의 단순성이 더 큰 가치가 될 수 있다.
