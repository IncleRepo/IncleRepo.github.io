...
`ORDER BY created_at DESC, id DESC`와 맞는 인덱스가 있으면 MySQL은 테이블 전체를 다시 정렬하지 않고 인덱스 순서를 따라갈 수 있다. 이때도 앞의 10만 개를 따라가며 건너뛰는 비용은 남는다.
...
Offset은 다음 요구사항에서 단순하고 좋은 선택이다.
...
Cursor로 다음 조회 위치를 전달할 수 있게 됐다. 여기에 전체 개수나 다음 페이지 여부까지 응답해야 한다면 어떤 결과 형식이 적절할까?
...
두 선택은 조합할 수 있다. `Slice`를 반환하면서 내부에서는 Offset을 사용하거나, 별도의 Cursor 응답을 반환하면서 Keyset으로 조회하는 식이다.
...
위처럼 `Slice`에 `Pageable`을 넘기면 전체 개수를 세는 Count 쿼리를 생략하면서 Offset으로 위치를 찾을 수 있다.
...
`Window`는 이번에 조회한 데이터와 각 데이터의 위치를 함께 제공한다. 애플리케이션은 마지막 위치를 다음 요청에 전달해 조회를 이어간다.
...
Count 비용은 데이터와 쿼리 조건에 따라 달라진다. 작은 테이블이나 단순한 조건에서는 충분히 빠르지만, 데이터가 크고 Join과 조회 조건이 복잡하며 결과 수가 많다면 비용이 커질 수 있다.
...
Count를 미리 계산해 두는 대신 원본과 집계 값을 함께 갱신하고, 동시 수정과 실패 후 복구를 처리해야 한다. 실제 Count가 병목인지 측정한 뒤 이 관리 비용을 감수할 가치가 있을 때 선택한다.
...
Keyset은 앞쪽에 새 행이 추가될 때 생기는 중복과 누락을 줄인다. 사용자가 페이지를 넘기는 사이 기존 행의 정렬 값이 바뀌면 Cursor 앞뒤로 이동할 수 있고, 행이 삭제돼도 이후 결과는 달라질 수 있다.
...
화면의 탐색 방식에 맞는 후보를 정한 뒤 깊은 페이지의 실행 계획과 실제 시간을 비교한다. 페이지 번호가 필요한 화면이라면 Offset의 단순함을 유지하면서 조회 범위와 Count 비용부터 줄일 수 있다.
...
**Offset과 Keyset은 페이지 번호가 필요한지, 깊은 페이지 조회가 실제 병목인지, 데이터를 조회하는 동안 얼마나 자주 변경되는지를 함께 보고 선택한다.**
