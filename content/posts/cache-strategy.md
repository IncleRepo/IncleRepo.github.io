...
lastmod = 2026-08-10T13:41:30+09:00
draft = false
...
Cache Aside와 Read Through는 모두 Cache를 먼저 본다. 차이는 Cache 사용 여부가 아니라 **누가 Miss 처리 책임을 갖는지가 핵심이다.**
...
### Spring `@Cacheable`은 비슷한 호출 경험을 제공한다
...
자주 읽지 않는 값까지 Cache에 채우는 일을 줄일 수 있다. 대신 변경 직후 첫 조회는 Miss를 만나므로 읽기 비용이 늘어난다. 이 글에서는 다른 전략과 같은 비중으로 외우기보다, **쓰기 시 Cache를 채우지 않는 선택도 존재한다.** 이 정도로 이해하면 충분하다.
...
Stampede 대응은 TTL 분산, Lock, 사전 갱신, 이전 값 제공처럼 응답 지연과 구현 복잡도가 서로 다른 선택지로 이루어진다. 하지만 이런 보호 장치도 캐시가 충분한 이득을 준다는 전제에서만 의미가 있다.
