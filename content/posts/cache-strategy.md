...
DB가 바뀐 직후에도 Cache에는 이전 값이 남을 수 있다. 조회 결과를 재사용하려면 **원본이 바뀐 뒤 Cache를 어떻게 갱신할지**도 정해야 한다.
...
캐시에 값이 있어 바로 반환하면 `Cache Hit`, 값이 없어 DB에서 가져오면 `Cache Miss`다.

Miss가 발생한 첫 요청은 Cache와 DB를 모두 거치고 Cache 저장까지 수행한다. 같은 Key가 다시 조회되어 Hit가 발생할 때부터 앞에서 지불한 적재 비용을 회수할 수 있다.

따라서 캐시는 **반복되는 요청에서 이전 결과를 재사용해 원본 조회를 줄이는 장치다.** 이제 이 흐름을 애플리케이션 코드로 직접 구현하면 어떤 모습인지 살펴보자.
...
Service가 Cache를 확인하고, 값이 없으면 DB 조회와 적재까지 직접 처리한다. 이처럼 애플리케이션이 두 저장소 사이의 흐름을 관리하는 방식을 `Cache Aside`라고 한다.
...
흐름이 단순해 실무에서 자주 선택하지만, DB 변경과 Cache 제거 사이에는 값이 어긋나는 구간이 생길 수 있다.
...
삭제가 실패했다면 사용자는 TTL이 끝날 때까지 이전 값을 볼 수 있다. 10분의 TTL은 DB와 Cache의 동기화 주기가 아니라 이전 값이 남을 수 있는 최대 시간이다.

따라서 TTL은 **오래된 Cache가 영원히 남는 것을 막는 마지막 안전장치**이며, 즉시 정합성은 별도의 무효화 흐름으로 확보해야 한다.
...
캐시 전략은 읽기 성능과 함께 **원본과 Cache가 어긋난 상태를 얼마나 오래 허용할지 결정하는 일**이다.
...
Cache Aside에서 Service가 맡던 **Miss 처리 책임을 Cache 계층으로 옮긴 것**이다. 호출 코드는 단순해지고, Cache 계층이나 Provider가 Loader를 관리한다.
...
Miss 처리가 호출 코드 밖으로 숨겨진다는 점에서는 Read Through와 비슷하다. 전통적인 Read Through에서는 Cache Provider가 원본 저장소를 읽지만, `@Cacheable`에서는 원본을 읽는 애플리케이션 메서드가 Loader와 비슷한 역할을 한다.

Spring Cache는 이 호출 흐름을 공통 API로 제공한다. 실제 저장 방식과 TTL, 분산 환경의 동작은 Redis나 Caffeine 같은 구현체와 설정으로 정한다.
...
Redis는 값을 저장하는 기술이고, Write Through는 애플리케이션이나 Cache Provider가 쓰기 흐름을 어떻게 구성했는지 설명하는 전략이다. Redis를 사용하더라도 쓰기 흐름을 별도로 구성해야 한다.
...
Write Behind에서는 **사용자에게 성공했다고 응답한 뒤에도 DB 반영이 끝나지 않은 상태**가 생긴다. 이 상태의 유실과 재시도를 책임지는 구조가 필요하다.
...
재고나 결제처럼 원본 정합성이 중요한 데이터에는 별도의 내구성 장치와 복구 설계가 필요하다. 업무가 요구하는 보장 수준을 만족할 수 있을 때 Write Behind를 선택한다.
...
쓰기
→ DB만 반영
→ Cache에는 새 값을 저장하지 않음

기존 Cache가 없거나 제거된 경우
→ 다음 조회에서 Cache Miss
→ DB 조회
→ Cache 적재

기존 Cache가 남아 있는 경우
→ 다음 조회에서 예전 값으로 Cache Hit 가능
```

따라서 Write Around를 적용할 때도 기존 Cache가 남아 있을 가능성을 확인해야 한다. DB를 변경하면서 Cache를 제거했다면 다음 조회는 다음과 같이 동작한다.
...
자주 읽지 않는 값까지 Cache에 채우는 일을 줄일 수 있다. 대신 Cache를 제거한 뒤 처음 들어온 조회는 Miss를 만나므로 읽기 비용이 늘어난다. 기존 Cache를 제거하지 않으면 새 값이 아니라 예전 값을 반환할 수 있다. 다른 전략과 같은 비중으로 외우기보다, **쓰기 시점에는 Cache를 채우지 않되 기존 값의 무효화는 별도로 책임져야 한다**고 이해하면 충분하다.
...
| Write Around | 애플리케이션 | DB에 직접 반영 | 기존 Cache 무효화와 이후 첫 조회의 Miss |
...
읽기와 쓰기에 서로 다른 전략을 조합했다면, 각 경로에서 Cache가 비거나 고장 났을 때 어디로 요청을 이어갈지도 정해야 한다.
...
이 구조의 핵심은 **각 단계가 실패했을 때 다음으로 어디에서 읽을지 정했다는 점**이다.

### 사용자 응답과 Cache 복구를 분리할 수 있다

사용자 응답과 1차 Cache를 다시 채우는 작업은 분리할 수 있다.
...
인기 Key가 만료된 순간 같은 값을 다시 만들려는 요청이 원본 저장소로 몰리는 현상을 **Cache Stampede**라고 한다. 여러 요청이 한꺼번에 몰리는 모습을 두고 `Thundering Herd`라는 표현도 함께 쓴다. 이 예에서는 1,000개의 요청이 같은 DB 조회와 계산을 반복하면서 부하가 급증한다.
...
대기와 Lock 관리 비용을 줄일 수 있는 다른 방법부터 확인해 보자.
...
확률적 조기 갱신은 **모든 요청이 정확한 만료 순간에 원본으로 몰리지 않도록 갱신 시점을 만료 전 구간에 분산한다.**
...
이 순서는 캐시를 도입할 때 빠뜨리기 쉬운 책임과 실패 상황을 확인하는 질문 목록으로 사용한다.
...
전략을 정할 때는 원본과 Cache가 어긋나도 되는 시간, 실패 후 다시 읽을 위치와 동시 Miss로부터 원본을 보호할 방법을 함께 선택한다.
