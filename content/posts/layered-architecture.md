...
하지만 업무 규칙만 나눈다고 Service의 변경 이유가 모두 줄어드는 것은 아니다. 계층 사이에서 어떤 객체를 주고받느냐에 따라 HTTP 요청이나 데이터 저장 방식의 변경까지 Service에 모일 수 있다.
...
같은 DTO와 Entity를 여러 계층이 공유하면 해당 객체의 변경도 함께 퍼질 수 있다.
