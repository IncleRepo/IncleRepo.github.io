...
lastmod = 2026-08-24T23:45:00+09:00
draft = false
...
API 변경이 Service까지 반복해서 번지거나 같은 Service를 배치와 메시지 소비자도 호출한다면 데이터 구조를 나눌 이유가 생긴다. Controller는 요청 DTO를 Service 입력값으로 바꾸고, Service는 업무에 필요한 값만 다루도록 정리할 수 있다. **변경이 반복해서 넘어오는 경계에서만 타입을 분리하면 된다.**
