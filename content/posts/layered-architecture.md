...
이 메서드는 주문 조회, 상태 검증, 변경만 조정한다. 여기에 가격 계산, 할인 판단, 재고 확인까지 더해지면 `OrderService`는 여러 업무 규칙을 차례로 실행하는 긴 코드가 될 수 있다.
...
이런 한계가 있더라도 레이어드 아키텍처의 단순함은 분명한 장점이다. 다음과 같은 상황에서는 Controller, Service, Repository만으로도 충분할 수 있다.
