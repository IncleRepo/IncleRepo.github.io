...
lastmod = 2026-08-24T21:35:00+09:00
draft = false
...
### 얇은 메서드보다 전체 요청의 비율을 본다

Service 메서드가 짧아도 트랜잭션, 인가나 여러 작업의 조정처럼 분명한 책임이 있을 수 있다. Service를 일관된 진입점으로 유지하면 Controller가 데이터 접근 방식에 직접 의존하지 않고, 나중에 업무 규칙이 추가돼도 같은 위치에서 처리할 수 있다.

따라서 메서드 하나의 길이보다 **애플리케이션 전체에서 단순 전달 요청이 차지하는 비율**을 확인한다. 《소프트웨어 아키텍처 101》에서는 다음과 같은 경험적 기준을 제시한다.
