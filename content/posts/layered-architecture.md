...
레이어드 아키텍처에서는 보통 상위 계층이 바로 아래 계층을 거쳐야 한다. 이러한 계층을 **닫힌 계층**이라고 한다.

Controller가 Repository를 직접 호출하지 않고 Service를 거치는 구조도 여기에 해당한다.
...
**메서드 하나보다 요청 흐름 전체에서 단순 전달이 차지하는 비중을 본다.**

레이어드 아키텍처에는 위와 같이 얇은 Service 메서드가 일부 생길 수 있다. 트랜잭션이나 인가처럼 Service가 맡은 책임이 다른 메서드에 드러나기도 한다.

Service를 일관된 진입점으로 유지하면 Controller가 데이터 접근 방식에 직접 의존하지 않는다. 나중에 트랜잭션이나 업무 규칙이 추가되어도 같은 위치에서 처리할 수 있다. Service가 트랜잭션, 인가나 여러 작업의 조정을 맡고 있다면 코드가 짧더라도 단순 전달 계층이라고 보기 어렵다.

**전체 요청 흐름을 기준으로 판단한다.**

판단해야 할 대상은 메서드 하나가 아니라 **애플리케이션의 전체 요청 흐름**이다. 《소프트웨어 아키텍처 101》에서는 다음과 같은 경험적 기준을 제시한다.

- 전체 요청의 약 20%만 단순 통과한다면 받아들일 만한 수준
- 반대로 약 80%가 단순 통과한다면 레이어드 아키텍처가 해당 문제에 잘 맞지 않는 신호

이 비율은 현재 구조가 문제에 잘 맞는지 점검하기 위한 경험적 기준이다.

**구조를 바꾼다면 팀의 규칙으로 정한다.**
...
Controller에서 Repository를 직접 호출하는 방식은 단순 조회처럼 범위를 명확히 정했을 때 선택할 수 있다. 메서드마다 개발자가 임의로 계층을 건너뛰지 않도록 허용 범위를 팀의 의존 규칙으로 관리해야 한다.

> **핵심:** 메서드 하나의 두께보다 단순 전달이 전체 요청에서 차지하는 비중과 각 계층의 실제 책임을 기준으로 판단한다.
...
Controller, Service와 Repository로 요청 처리, 업무 흐름과 데이터 접근의 책임은 나눴다. 다음 단계에서는 비즈니스 모델, 기술 의존성과 모듈 경계를 별도로 설계해야 한다.
...
레이어드 아키텍처로 책임을 나눈 뒤에는 **비즈니스 모델을 설계하고 외부 기술로부터 핵심 로직을 보호하는 문제를 별도로 다뤄야 한다.**
...
- DTO는 별도의 계층이 아니라 외부 또는 계층 사이에서 데이터를 전달하는 객체다.
- 계층 분리와 HTTP나 JPA 같은 기술과의 결합을 통제하는 일은 서로 다른 설계 과제다.
- Architecture Sinkhole은 얇은 Service 메서드 하나가 아니라, 대부분의 요청이 의미 있는 처리 없이 계층을 통과하는 구조를 가리킨다.
- 대부분의 요청이 별다른 처리 없이 계층을 통과한다면 구조를 다시 검토한다. 단순 조회에서 Service를 생략하더라도 허용 범위와 의존 규칙을 팀에서 함께 정해야 한다.
