...
Spring을 처음 배우면 높은 확률로 다음과 같은 프로젝트 구조를 만나게 된다.
...
패키지는 이 책임 분리를 코드에 표현하는 한 가지 수단이다. 그리고 DTO는 Controller나 Service 같은 계층이 아니다. **DTO**(Data Transfer Object)는 외부 또는 계층 사이에서 데이터를 전달하기 위한 객체다.
...
같은 회원 조회라는 작업 안에서도 요청 처리, 업무 흐름과 데이터 접근이 서로 다른 코드에 놓인다. 이 역할에 일반적으로 사용하는 계층 이름을 붙이면 다음과 같다.
...
자료에 따라 Business Layer와 Application Layer를 구분하고 별도의 Domain Layer를 두기도 한다. 여기서는 입문을 위해 Service가 위치한 영역을 Business/Application Layer로 단순화해서 보자. Database도 별도 계층으로 그리기도 하고, Persistence Layer 바깥의 저장소로 보기도 한다.
...
이처럼 코드 참조가 대체로 위에서 아래를 향하도록 제한하는 것이 레이어드 아키텍처에서 흔히 볼 수 있는 의존 방향이다. 바로 아래 계층만 호출하도록 엄격하게 제한하는 방식도 있고, 필요한 경우 더 아래 계층을 건너뛰어 호출하도록 허용하는 방식도 있다.

다만 참조 방향이 한쪽이라고 변경 영향까지 한쪽으로만 흐르는 것은 아니다. Repository의 메서드 반환형을 바꾸면 이를 사용하는 Service도 수정해야 할 수 있다. 의존 방향의 목적은 변경을 없애는 것이 아니라, **누가 누구를 참조하는지 예측할 수 있게 만들어 구조를 단순하게 유지하는 데 있다.**

## 이 구조가 널리 사용되는 이유

레이어드 아키텍처가 익숙한 데에는 분명한 이유가 있다.
...
Controller 하나에서 HTTP 요청 값을 읽고, 업무 조건을 검사하고, SQL을 실행한 뒤 JSON 응답까지 만든다고 생각해보자. 기능이 조금만 늘어도 수정 이유가 서로 다른 코드가 한곳에 뒤섞인다.
...
Controller와 Repository에서 코드를 분리했어도 Service 안에서는 `조회 → if문 → setter → 저장`이라는 절차가 계속 길어진다. 레이어드 아키텍처는 코드의 기술적인 책임을 어디에 둘지는 알려주지만, **복잡한 업무 규칙을 어떤 객체와 모델로 표현할지까지 정해 주지는 않는다.**

따라서 Service가 비대해지는 현상은 레이어드 아키텍처가 잘못됐다는 뜻이 아니다. 계층 분리와 도메인 모델링은 서로 다른 문제라는 뜻에 가깝다.

## 계층을 나눠도 기술 의존성이 안쪽까지 퍼질 수 있다

그림에서 Database가 가장 아래에 있다고 해서 프로젝트가 자동으로 DB 중심 설계가 되는 것은 아니다. 실제로 살펴봐야 하는 것은 비즈니스 코드에 구체적인 저장 기술이 얼마나 직접 드러나는가다.
...
위 Service는 이름부터 JPA를 드러내는 Repository를 직접 알고 있다. 모든 계층이 같은 JPA Entity를 공유하거나 Repository의 조회 결과 형태에 맞춰 업무 코드가 바뀐다면, 패키지는 나뉘어 있어도 실제 결합은 강할 수 있다.

Web 요청 DTO를 Service와 데이터 접근 코드까지 그대로 전달하는 경우도 비슷하다.
...
그러면 API 요청 형식이 바뀌었을 뿐인데 안쪽 코드까지 함께 수정해야 할 수 있다. 즉, 계층을 나눈 것과 계층 사이에서 데이터 구조와 구현 기술을 분리한 것은 같은 일이 아니다.

애플리케이션이 필요로 하는 Repository 계약을 인터페이스로 정의하고 실제 저장 기술이 이를 구현하게 만들면 이런 의존성을 줄이는 데 도움이 된다. 하지만 인터페이스 하나를 추가했다고 모든 결합이 사라지는 것은 아니다. 계약의 위치, Entity 공유 범위와 의존 방향까지 함께 설계해야 한다. 이 문제를 더 엄격하게 다루려는 고민은 이후 다른 아키텍처로 이어진다.
...
그렇다고 얇은 Service를 발견하는 즉시 없애야 하는 것은 아니다. 트랜잭션, 인가와 일관된 호출 경계를 유지하기 위해 얇은 Service를 허용하는 팀도 있다. 중요한 것은 모든 요청이 무조건 모든 계층을 지나야 한다고 생각하기보다, **그 계층이 맡은 책임이 실제로 있는지 확인하는 일**이다.
...
아키텍처는 유명한 이름이 아니라 현재 문제의 크기에 맞춰 선택해야 한다. 단순한 문제에는 단순한 구조가 더 좋은 답일 수 있다.
...
이 질문들은 레이어드 아키텍처가 틀렸다는 뜻이 아니다. **계층 분리만으로는 비즈니스 모델을 설계하는 방법이나 외부 기술로부터 핵심 로직을 보호하는 방법까지 자동으로 얻을 수 없다는 뜻**이다.
...
- 계층만 나눈다고 좋은 도메인 모델이나 낮은 기술 의존성이 자동으로 만들어지지는 않는다.
- 모든 요청이 모든 계층을 형식적으로 통과해야 하는 것은 아니며, 각 계층이 실제 책임을 맡고 있는지 살펴봐야 한다.

Controller-Service-Repository 구조가 익숙한 이유는 코드를 기술적인 책임에 따라 나누기 쉽기 때문이다. 이 구조는 단순하고 이해하기 쉽다는 큰 장점이 있다. 다만 시스템이 복잡해질수록 “**계층을 나누는 것 다음에는 무엇을 고민해야 할까**?”라는 질문이 생긴다.
